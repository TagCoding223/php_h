...
?>
    ...
    <div class="container my-4">
        ...
        <?php
                $id=$_GET['catid'];
                $start_index=$_GET['start_page'];
                $end_index=$start_index+10;
                $q_sql="SELECT * FROM `threads` WHERE `thread_cat_id`='$id'";
                
                    $q_result=mysqli_query($con,$q_sql);
                    $arr_data=mysqli_fetch_all($q_result,MYSQLI_ASSOC); // this is not a good way when records is large in number
                    // echo '<pre>';
                    // print_r($arr_data);
                    
                    while($start_index<$end_index && ($arr_data!=NULL) && $start_index<count($arr_data)){
                            ...
                            <h5 class="mt-2"><a href="./thread.php?thread_id='.str_desalinization($arr_data[$start_index]['thread_id']).'">'.str_desalinization($arr_data[$start_index]['thread_title']).'</a></h5>
                            '.substr((str_desalinization($arr_data[$start_index]['thread_desc'])),0,120).'...</div></div><hr>
                    ...
                    if(count($arr_data)!=0){
                        echo '<div class="container ml-2 pr-5 my-2" style="height:5vh;">';
                        if($_GET['start_page']!=0){
                                $now=$_GET['start_page'];
                                $start_page=($now-10);
                                echo '
                                <a href="./thread_list.php?catid='.$row['category_id'].'&start_page='.$start_page.'"><button type="button" class="btn btn-primary"><i class="fa fa-mail-reply"></i> Previous</button></a>';
                            }
                        if($end_index+1<count($arr_data)) {
                                $start_page=$end_index;
                                echo '
                                    <a href="./thread_list.php?catid='.$row['category_id'].'&start_page='.$start_page.'"><button type="button" class="btn btn-primary float-right">Next <i class="fa fa-mail-forward"></i></button></a>';
